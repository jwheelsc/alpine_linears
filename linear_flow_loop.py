...
## %% this is the raster of drig cell i and j
...
for jj in np.arange(21):
    for ii in np.arange(21):
        ...
        # %% this is the raster with the nice crack
        ...
        # %% add a very small random number to all elevations so that they are all unique
        #gH = gH+(np.random.default_rng().integers(low=1,high=9)*0.001)
        gH = gH+np.random.rand(np.shape(gH)[0],np.shape(gH)[1])*0.001
        ...
        # %% note that this version of the script is a cumbersome way of doing the section above, 
        # % but I'm holding onto it for some reason
        ...
        saveFold = 'C:\\Users\\jcrompto\\Documents\\code\\python_scripts\\detect_alpine_linears\\saveCracks\\Joffre\\'
        ...
        chordRatio = np.zeros(numCracks)
        keepCrack = np.array(0,dtype = int)
        ...
        # crk = 4
            ...
            if len(c_xU)>3:
                ...
                lenFit = np.sum(np.sqrt((np.square(np.diff(c_xU))+np.square(np.diff(pFit)))))
                asEagle = np.sqrt(np.square(c_xU[-1]-c_xU[0]) + np.square(pFit[-1]-pFit[0]))
                chordRatio[crk] = np.divide(lenFit,asEagle)
            ...
            if aveSlopeCrack[crk]<0.35 and r2[crk]>0.8 and chordRatio[crk]<1.2:
                slpArr.append(aveSlopeCrack[crk])
                r2Arr.append(r2[crk]) 
                chordArr.append(chordRatio[crk])
                keepCrack = np.hstack((keepCrack,crk))
                crackListFinal.append(crackList[crk])
                yCoords, xCoords = DEM_rast.ij2xy(i=[c_y], j=[c_x])
                realCrackCoords = np.hstack((np.rot90(yCoords),np.rot90(xCoords)))
                # coordList.append(np.squeeze(np.hstack((yCoords,xCoords))))
                coordListX.append(np.squeeze(xCoords))
                coordListY.append(np.squeeze(yCoords))
                #plt.plot(c_xU,pFit,'g-o')
                #plt.plot(np.flipud(expandedCrackU[:,1]),np.flipud(expandedCrackU[:,0]),'g.')
        
        if np.size(keepCrack)==1:
            slpArr=0
            r2Arr=0 
            chordArr=0
            coordListXSave = 0
            coordListYSave = 0
            strCrk = saveFold + 'keepCracks_' + str(jj) + '_' + str(ii) 
            np.savez(strCrk,coordListXSave,coordListYSave)
            strCrk_stats = saveFold + 'keepCracks_' + str(jj) + '_' + str(ii) + '_stats'
            np.savez(strCrk_stats,slpArr,r2Arr,chordArr)
            
        else:   
            keepCrack = keepCrack[1:]
            # strCrkcsv = saveFoldTile + 'keepCracks' + str(jj) + '_' + str(ii) + '.csv'
            strCrk = saveFold + 'keepCracks_' + str(jj) + '_' + str(ii) 
            strCrk_stats = saveFold + 'keepCracks_' + str(jj) + '_' + str(ii) + '_stats'
            # np.savetxt(strCrkcsv, realCrackCoords, delimiter = ',')
            coordListXSave = np.asarray(coordListX, dtype="object")
            coordListYSave = np.asarray(coordListY, dtype="object")
            np.savez(strCrk,coordListXSave,coordListYSave)
            np.savez(strCrk_stats,slpArr,r2Arr,chordArr)
